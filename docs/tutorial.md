# Reporting OPA Gatekeeper audit violations in Security Command Center
...
policies related to security, regulations, or business rules.

The
[audit functionality](https://open-policy-agent.github.io/gatekeeper/website/docs/audit)
in OPA Gatekeeper allow you to implement detective controls. The audit
controller periodically evaluates resources against policies and creates
violations for resources that don't conform to the policies. These violations
are stored in the cluster, and you can query them using Kubernetes tools such
as `kubectl`.
...
(GKE) cluster to synchronize OPA Gatekeeper constraint violations to findings
...
[tutorial on how to create policy-compliant Google Cloud resources using Config Connector](https://cloud.google.com/anthos-config-management/docs/tutorials/policy-compliant-resources).
...
-   Create a finding in Security Command Center from an OPA Gatekeeper policy
...
1.  <walkthrough-project-billing-setup></walkthrough-project-billing-setup>
    ...
    - [Go distribution](https://golang.org/dl/) version 1.19 or later
...
2.  Grant the cluster-admin cluster role to your Google user account:
    ...
    You need this role later to the install OPA Gatekeeper, and to deploy the
...
1.  Clone the
    ...
    repository, navigate to the repository directory, and check out a known
    commit:
...
The OPA Gatekeeper
[audit controller](https://open-policy-agent.github.io/gatekeeper/website/docs/audit)
periodically evaluates resources against constraints. This audit enables you
to detect policy-violating resources created prior to the constraint.
...
1.  View violations for all constraints by querying using the `constraint`
    ...
    This can happen because the audit hasn't run since you created the
    constraint. The audit runs every minute by default.

**Note:** OPA Gatekeeper have a
[default limit on the number of reported violations per constraint](https://open-policy-agent.github.io/gatekeeper/website/docs/audit#configuring-audit).
...
Follow these steps to create a source for findings from OPA Gatekeeper.
...
You can create Security Command Center findings from OPA Gatekeeper constraint
violations using the `gatekeeper-securitycenter` tool as part of a build
pipeline or scheduled task.
...
2.  Install [`kpt`](https://googlecontainertools.github.io/kpt/):
    ...
    If you _don't_ use Cloud Shell, install `kpt` using `gcloud`:
    ...
    [`kpt`](https://kpt.dev/) is a command-line tool that allows you to manage,
    manipulate, customize, and apply Kubernetes resources. You use `kpt` in
    this tutorial to customize the resource manifests for your environment.
...
5.  Set the cluster name. This step is optional, and you can use any name you
    ...
    **Note:** Other
    [setters](https://catalog.kpt.dev/list-setters/v0.1/)
    are available to customize the package. Run the command
    `kpt fn eval manifests --image gcr.io/kpt-fn/list-setters:v0.1.0`
    to list the available setters and their values.
...
8.  Apply the controller resources to your cluster:
    ...
    -   a cluster role binding that grants the cluster role to the
        service account
...
15. To verify that the controller can set the finding state to
    ...
    when a violation is no longer reported by OPA Gatekeeper, delete the pod
    called `opa-disallowed` in the `default` namespace:
...
1.  Use the `gcloud` tool to list findings for your organization and source:
    ...
    Some of the source properties will be missing (empty) for some
    ...
    [Config Connector](https://cloud.google.com/anthos-config-management/docs/tutorials/policy-compliant-resources)
...
1.  If the OPA Gatekeeper audit controller doesn't report violations in the
    `status` field of the constraint objects, you can view the controller logs:
...
2.  If the `gatekeeper-securitycenter` controller doesn't create findings in
    Security Command Center, you can view logs of the controller manager:
...
4.  If you get permission errors when using the `gatekeeper-securitycenter`
    command-line tool, configure your
...
5.  If you run into other problems with this tutorial, we recommend that you
...
-   Learn how to
    [create policy-compliant Google Cloud resources using Config Connector](https://cloud.google.com/anthos-config-management/docs/tutorials/policy-compliant-resources).

-   Discover how to
    [run validation as part of a continuous integration pipeline in Cloud Build](https://cloud.google.com/anthos-config-management/docs/tutorials/app-policy-validation-ci-pipeline).
...
-   Try out other Google Cloud features for yourself. Have a look at the
    [quickstarts and tutorials](https://cloud.google.com/docs/tutorials).
