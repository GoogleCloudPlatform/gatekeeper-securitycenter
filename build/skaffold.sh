...
if ! [ -x "$(command -v ko)" ]; then
    pushd $(mktemp -d)
    go mod init tmp; GOFLAGS= go get github.com/google/ko/cmd/ko@v0.7.2
    popd
fi

if ! [ -x "$(command -v crane)" ]; then
    pushd $(mktemp -d)
    go mod init tmp; GOFLAGS= go get github.com/google/go-containerregistry/cmd/crane@v0.4.0
    popd
fi
